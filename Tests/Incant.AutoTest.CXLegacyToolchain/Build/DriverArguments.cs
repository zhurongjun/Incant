...
internal static class DriverArguments
{
    internal static ArgumentSet Configuration(ResolvedToolchain toolchain)
    {
        ...
        if (toolchain.AdapterKind == BuildAdapterKind.Wasi)
        {
            arguments = arguments.WithExceptions(toolchain.Multilib == "eh"
                ? ExceptionMode.Wasm : ExceptionMode.Disabled);
    ...
    internal static IReadOnlyList<string> Generate(
        ResolvedToolchain toolchain, Operation operation, ArgumentSet arguments)
    {
        var driver = new ArgumentDriver(SelectDialect(toolchain), operation);
        ArgumentGenerationResult result = driver.Generate(arguments);
        if (!result.Success)
        {
            throw new InvalidOperationException(string.Join(Environment.NewLine,
                result.Diagnostics.Select(diagnostic =>
                    diagnostic.Field + ": " + diagnostic.Message + " Sources: "
    ...
    internal static IReadOnlyList<string> ArchiveArguments(
        ResolvedToolchain toolchain, ArchiveMode mode, string archive, IReadOnlyList<string>? inputs = null, bool indexer = false) =>
        Generate(toolchain, Operation.Archive, Configuration(toolchain)
            .WithArchiveDialect(indexer ? ArchiveDialect.Ranlib : toolchain.Archiver.Name is "lib" or "llvm-lib"
                ? ArchiveDialect.Msvc : ArchiveDialect.Gnu)
            .WithArchiveMode(mode).WithOutput(archive).WithInputs(inputs ?? []));

    private static Dialect SelectDialect(ResolvedToolchain toolchain) => toolchain.AdapterKind switch
    {
        BuildAdapterKind.Msvc => Dialect.Msvc,
        BuildAdapterKind.ClangCl => Dialect.ClangCl,
        BuildAdapterKind.Gnu => Dialect.Gnu,
        BuildAdapterKind.Llvm => Dialect.Clang,
        BuildAdapterKind.Apple => Dialect.AppleClang,
        BuildAdapterKind.Android => Dialect.AndroidClang,
        BuildAdapterKind.Emscripten => Dialect.Emscripten,
        BuildAdapterKind.Wasi => Dialect.WasiClang,
