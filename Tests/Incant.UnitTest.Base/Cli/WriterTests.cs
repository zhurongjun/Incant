...
public sealed class WriterTests
{
    ...
    [Fact]
    public void WriteKeepIndentAtLineStartPreservesTextWithoutAddedSpaces()
    {
        var writer = new Writer();

        writer.WriteKeepIndent("first\nsecond");

        Assert.Equal("first\nsecond", writer.Content);
    }

    [Fact]
    public void WriteKeepIndentUsesColumnAfterEmbeddedNewLine()
    {
        var writer = new Writer();

        writer.Write("prefix\nxy").WriteKeepIndent("first\nsecond");

        Assert.Equal("prefix\nxyfirst\n  second", writer.Content);
    }

    [Fact]
    public void WriteKeepIndentPreservesCarriageReturnNewLines()
    {
        var writer = new Writer();

        writer.Write("  ").WriteKeepIndent("first\r\nsecond");
    ...
    [Fact]
    public void FluentMethodsReturnTheSameWriter()
    {
        var writer = new Writer();

        Assert.Same(writer, writer.Write(string.Empty));
        Assert.Same(writer, writer.WriteKeepIndent(string.Empty));
        Assert.Same(writer, writer.WriteLine(string.Empty));
        Assert.Same(writer, writer.NextLine());
        Assert.Same(writer, writer.StyleBold());
    }
}
