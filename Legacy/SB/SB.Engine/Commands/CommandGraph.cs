...
public class GraphCommand : CommandBase
{
    ...
    public string OutputDirectory { get; set; } = Path.Combine(Directory.GetCurrentDirectory(), "build", ".graphs");
    ...
    [Cli.Option(Name = "only-positive", Help = "Only include PositiveBuild root targets in the graph", IsRequired = false)]
    ...
    public override int OnExecute()
    {
        ...
        var emitter = Instance.AddTaskEmitter("DependencyGraph", new DependencyGraphEmitter
        {
            Format = ParseFormat(Format),
            ColorScheme = ParseColorScheme(ColorScheme),
            OutputDirectory = OutputDirectory,
            IncludeExternalDeps = IncludeExternalDeps,
            ShowTransitiveDeps = ShowTransitiveDeps,
            MaxDepth = MaxDepth
        });

        // Run the build pipeline to collect dependency information for all loaded targets.
        RunBuildForAllTargets(Instance, OnlyPositive);

        // Generate the dependency graph after build
        emitter.GenerateDependencyGraph();
