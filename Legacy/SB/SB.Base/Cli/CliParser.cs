...
public class ParseContext
{
    ...
    public Writer Writer { get; init; } = new Writer();
    public bool AnyError { get; private set; } = false;
    public bool AnyWarning { get; private set; } = false;
    public HashSet<IOption> VisitedOptions { get; private set; } = new();
...
public class CommandParser
{
    ...
    private bool _PrintHelpIfMeetHelpOption(Command cmd, Token token, ParseContext ctx)
    {
        if (token.IsOptionOf("help") || token.IsShortOptionOf('h'))
        {
            ...
            // print help
            cmd.WriteHelp(ctx.Writer, Banner);
    ...
    private bool _InvokeCommand(Command cmd, ParseContext ctx)
    {
        ...
        {
            // collect lost required options
            List<IOption> lostRequiredOptions = new();
            foreach (var option in cmd.Options)
            {
                if (option.IsRequired && !ctx.VisitedOptions.Contains(option))
                {
            ...
            {
                var lostOptionsStr = string.Join(", ", lostRequiredOptions.Select(o => $"--{o.Name}"));
                ctx.Error($"Lost REQUIRED options: [{lostOptionsStr}]");
            }
        }
    ...
    private void _ParseOption(Command cmd, Token token, ParseContext ctx)
    {
        ...
        if (option == null)
        {
            _HandleUnrecognizedOption(token, optionName, ctx);
        }
        else
        {
            if (option.IsToggle)
    ...
    private void _ParseShortOption(Command cmd, Token token, ParseContext ctx)
    {
        ...
        { // means toggle group
            if (optionValue != null)
            {
                ctx.Error($"Toggle group -{optionNames} in '{token.Raw}' cannot have a value");
            }
    ...
    private void _AssignOption(IOption option, string? optionValue, Token token, ParseContext ctx)
    {
        ...
        {
            // handle selections
            var selection = option.Selections;
            if (selection != null && !selection.Contains(optionValue))
            {
                ctx.Error($"Option '{option.Name}' with value '{optionValue}' is not in selections");
                return;
            }
    ...
    {
        string msg = $"Unrecognized argument: {token.Raw}";
        if (FailedWhenUnrecognizedArg)
            ctx.Error(msg);
        else
            ctx.Warning(msg);
    }
    private void _HandleUnrecognizedShortOption(Token token, char shortOptionName, ParseContext ctx)
    {
        string msg = $"Unrecognized short option: -{shortOptionName} in '{token.Raw}'";
        if (FailedWhenUnrecognizedOption)
            ctx.Error(msg);
        else
            ctx.Warning(msg);
    }
    private void _HandleUnrecognizedOption(Token token, string optionName, ParseContext ctx)
    {
        string msg = $"Unrecognized option: --{optionName} in '{token.Raw}'";
        if (FailedWhenUnrecognizedOption)
        {
            ctx.Error(msg);
        }
    ...
    {
        string msg = $"Missing argument: --{option.Name} in '{token.Raw}'";
