...
public class ReflRestOptions : IRestOption
{
    ...
    public ReflRestOptions(PropertyInfo prop, object instance, RestOptionsAttribute attr)
    {
        ...
        // check property type
        if (
            _Property.PropertyType != typeof(List<string>) &&
    ...
    public void Assign(ParseContext ctx, List<string> values)
    {
        ...
        else if (_Property.PropertyType == typeof(string))
        {
            if (values.Count > 1)
            {
                ctx.Error($"RestOptions property '{_Property.Name}' is of type string, but got multiple values");
            }
            else if (values.Count == 0)
            {
                if (_Attribute.IsRequired)
                {
                    ctx.Error($"RestOptions property '{_Property.Name}' is required, but got no values");
                }
                else
                {
                    _Property.SetValue(_Instance, string.Empty);
                }
            }
            else
            {
                _Property.SetValue(_Instance, values[0]);
            }
        }
        else
        {
