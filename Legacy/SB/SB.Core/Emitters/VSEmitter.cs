...
namespace SB
{
    ...
    public class VSEmitter : TaskEmitter
    {
        ...
        public int SolutionFolderDepth { get; set; } = 2;
        public string? OutputDirectory { get; set; }
        private readonly ConcurrentDictionary<string, VSProjectInfo> _projectInfos = new();
        ...
        public override IArtifact? PerTargetTask(BuildInstance Instance, Target Target)
        {
            ...
            _projectInfos.TryAdd(Target.Name, projectInfo);
        ...
        private void ExtractCompileArguments(Target Target, VSProjectInfo projectInfo)
        {
            ...
            var driver = Toolchain.Compiler.CreateArgumentDriver(Target.Instance, CFamily.Cpp, false)
        ...
        {
            if (!_projectInfos.TryGetValue(Target.Name, out var projectInfo))
        ...
        public void GenerateSolution(string solutionPath, string solutionName)
        {
            ...
            // Add projects
            foreach (var kvp in _projectInfos)
            {
                var name = kvp.Key;
                var info = kvp.Value;
            ...
            foreach (var info in _projectInfos.Values)
            {
        ...
        private void CollectFolderPaths(HashSet<string> allFolderPaths)
        {
            foreach (var projectInfo in _projectInfos.Values)
            {
        ...
        private (string, string)[] GetNMakeProperties(Target target, string config, string platform, string outDir)
        {
            ...
            var buildCommand = $"cd \"{RootDirectory}\" && dotnet run --project Legacy/SB/SB.csproj -- build --mode {config.ToLower()} {target.Name}";

            return new[]
            {
                ("NMakeBuildCommandLine", buildCommand),
                ("NMakeReBuildCommandLine", buildCommand),
                ("NMakeCleanCommandLine", $"cd \"{RootDirectory}\" && dotnet run --project Legacy/SB/SB.csproj -- clean"),
