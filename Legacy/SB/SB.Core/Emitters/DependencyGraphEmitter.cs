...
namespace SB
{
    public class DependencyGraphEmitter : TaskEmitter
    {
        ...
        public GraphFormat Format { get; set; } = GraphFormat.DOT;
        public NodeColorScheme ColorScheme { get; set; } = NodeColorScheme.ByType;
        public bool IncludeExternalDeps { get; set; } = false;
        public bool ShowTransitiveDeps { get; set; } = false;
        public int MaxDepth { get; set; } = -1;

        private readonly Dictionary<string, TargetInfo> _targetInfos = new();
        ...
        public override bool EnableEmitter(BuildInstance Instance, Target Target) => true;
        ...
        public override IArtifact? PerTargetTask(BuildInstance Instance, Target Target)
        {
            ...
            lock (_syncLock)
            {
                _targetInfos[Target.Name] = info;
                foreach (var depName in deps)
                    _edges.Add((Target.Name, depName));
            }
        ...
        public void GenerateDependencyGraph()
        {
            ...
            lock (_syncLock)
            {
                targetsCopy = new Dictionary<string, TargetInfo>(_targetInfos);
                edgesCopy = new HashSet<(string from, string to)>(_edges);
            }
        ...
        private void GenerateDot(Dictionary<string, TargetInfo> targets, HashSet<(string from, string to)> edges, string path)
        {
        ...
        private void GenerateMermaid(Dictionary<string, TargetInfo> targets, HashSet<(string from, string to)> edges, string path)
        {
            var sb = new StringBuilder();
        ...
        private void GeneratePlantUML(Dictionary<string, TargetInfo> targets, HashSet<(string from, string to)> edges, string path)
        {
            var sb = new StringBuilder();
        ...
        private void GenerateStats(Dictionary<string, TargetInfo> targets, HashSet<(string from, string to)> edges, string path)
        {
            var sb = new StringBuilder();
        ...
        private string GetNodeLabel(string name, TargetInfo info)
        {
        ...
        private string GetNodeColor(TargetInfo info)
        {
