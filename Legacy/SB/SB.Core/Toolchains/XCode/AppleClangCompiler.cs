...
namespace SB.Core
{
    ...
    public class AppleClangCompiler : ICompiler, ILinker
    {
        public AppleClangCompiler(string ExePath, XCode Toolchain)
        {
            ...
            int ExitCode = BuildInstance.RunProcess(ExePath, "--version", out var Output, out var Error);
            if (ExitCode == 0)
            {
        ...
        IArgumentDriver ICompiler.CreateArgumentDriver(BuildInstance Instance, CFamily Language, bool isPCH)
            => new AppleClangArgumentDriver(Instance, XCodeToolchain.DeveloperDirectory!, XCodeToolchain.PlatSDKDirectory!, Version, Language, isPCH);
        ...
        public LinkResult Link(TaskEmitter Emitter, Target Target, IArgumentDriver Driver)
        {
            ...
            {
                int ExitCode = BuildInstance.RunProcess(ExecutablePath, String.Join(" ", LinkerArgsList), out var OutputInfo, out var ErrorInfo);
                if (ExitCode != 0)
        ...
        IArgumentDriver ILinker.CreateArgumentDriver(BuildInstance Instance) =>
            new ClangLinkerArgumentDriver(Instance, XCodeToolchain.PlatSDKDirectory!);
