...
namespace SB.Core
{
    public partial class XCode : IToolchain
    {
        ...
        internal void FindWithXCRun()
        {
            ...
            {
                BuildInstance.RunProcess("xcrun", "-sdk macosx --show-sdk-path", out string? output, out string? error);
                PlatSDKDirectory = output.Trim();
            }
            if (DeveloperDirectory is null)
            {
                BuildInstance.RunProcess("xcrun", "xcode-select --print-path", out string? output, out string? error);
                DeveloperDirectory = output.Trim();
                DeveloperDirectory = Path.Combine(DeveloperDirectory, "Toolchains/XcodeDefault.xctoolchain");
            }
            if (SDKVersion is null)
            {
                BuildInstance.RunProcess("xcrun", "-sdk macosx --show-sdk-version", out string? output, out string? error);
            ...
            if (!HasClang)
            {
                BuildInstance.RunProcess("xcrun", "-sdk macosx --find clang", out string? output, out string? error);
            ...
            if (!HasAR)
            {
                BuildInstance.RunProcess("xcrun", "-sdk macosx --find ar", out string? output, out string? error);
        ...
        public string? PlatSDKDirectory { get; private set; }
    ...
    {
        public XCode XCode { get; } = new XCode();
