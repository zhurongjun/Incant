    ...
    public class AR : IArchiver
    {
        ...
        public ArchiveResult Archive(TaskEmitter Emitter, Target Target, IArgumentDriver Driver)
        {
            ...
            bool Changed = BuildDatabase.GetCompileDatabaseForTarget(Target).RunIfOutdated(Target.Name, OutputFile!, Emitter.Name, (DependencyRecord depend) =>
            {
                ...
                int ExitCode = BuildInstance.RunProcess(ExecutablePath, ArgsString, out var OutputInfo, out var ErrorInfo);
                if (ExitCode != 0)
        ...
        public IArgumentDriver CreateArgumentDriver(BuildInstance Instance) => new ARArgumentDriver();
