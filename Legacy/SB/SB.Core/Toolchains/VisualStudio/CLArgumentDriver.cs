namespace SB.Core
{
    ...
    {
        public CLArgumentDriver(BuildInstance Instance, CFamily Language, bool isPCH)
        {
            ...
            {
                if (Instance.TargetArch == Architecture.X86)
                {
                    RawArguments.Add("-D_WIN32=1");
                }
                else if (Instance.TargetArch == Architecture.X64)
                {
                    RawArguments.Add("-D_WIN32=1");
        ...
        public static IEnumerable<string> ToCommandLineArguments(Dictionary<string, string[]> Args)
        {
            foreach (var (Name, Values) in Args)
            {
                foreach (var Value in Values)
                {
                    yield return Name switch
                    {
                        "Defines" => QuoteDefineForCommandLine(Value),
                        "IncludeDirs" => QuoteIncludeDirForCommandLine(Value),
                        _ => Value
                    };
                }
            }
        ...
        private static string QuoteDefineForCommandLine(string Argument)
        {
            // Defines() 返回的是逻辑 argv；只有在 RunProcess 拼接 Windows 命令行字符串时才补外层引用。
            // 路径宏里的 \" 用来表达 C/C++ 字符串字面量，外层引用前还原为普通引号，避免改变编译器最终收到的宏值。
            return Argument.Any(char.IsWhiteSpace) ? BS.QuoteCommandLineArgument(Argument.Replace("\\\"", "\"")) : Argument;
        }

        private static string QuoteIncludeDirForCommandLine(string Argument)
        {
            if (Argument.StartsWith("/I"))
            {
                var Path = Argument.Substring(2);
                return Path.Any(char.IsWhiteSpace) ? $"/I{BS.QuoteCommandLinePath(Path)}" : Argument;
            }
            return Argument;
        }
    }
