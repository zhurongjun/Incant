    ...
    public class ClangCLCompiler : ICompiler
    {
        public ClangCLCompiler(string ExePath, Dictionary<string, string?> Env)
        {
           ...
           ProcessOptions Options = new ProcessOptions
            ...
            int ExitCode = BuildInstance.RunProcess(ExePath, "--version", out var Output, out var Error, Options);
            if (ExitCode == 0)
            {
        ...
        public IArgumentDriver CreateArgumentDriver(BuildInstance Instance, CFamily Language, bool isPCH)
        {
            return new ClangCLArgumentDriver(Instance, Language, isPCH);
        }
        ...
        public CompileResult Compile(TaskEmitter Emitter, Target Target, IArgumentDriver Driver, string? WorkDirectory = null)
        {
            ...
            var LogicalCompilerArgsList = CompilerArgsDict.Values.SelectMany(x => x).ToList();
            var CompilerArgsList = Driver is CLArgumentDriver
                ? CLArgumentDriver.ToCommandLineArguments(CompilerArgsDict).ToList()
                : LogicalCompilerArgsList;
            var DependArgsList = LogicalCompilerArgsList.ToList();
            ...
            var Changed = BuildDatabase.GetCompileDatabaseForTarget(Target).RunIfOutdated(Target.Name, FileToCompile!, Emitter.Name, (DependencyRecord depend) =>
            {
                ProcessOptions Options = new ProcessOptions
                {
                    Environment = VCEnvVariables,
                ...
                int ExitCode = BuildInstance.RunProcess(ExecutablePath, String.Join(" ", CompilerArgsList), out var OutputInfo, out var ErrorInfo, Options);
                if (ExitCode != 0)
                {
